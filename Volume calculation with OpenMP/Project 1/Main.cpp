...
int main(int argc, char *argv[])
{
	...
	printf("Using %d threads and %d nodes. \n", NUMT, NUMNODES);
	...
	for (int i = 0; i < NUMNODES*NUMNODES; i++)
	{
		...
		else if (iu == 0 && iv == 0 || iu == (NUMNODES - 1) && iv == 0 || iu == 0 && iv == (NUMNODES - 1) || iu == (NUMNODES - 1) && iv == (NUMNODES - 1)) {
	...
	printf("area is %f , time is %10.2lf , MegaHeights computed per second is %f\n", volume, 1000000. * (time1 - time0), MegaHeightsPerSec);
